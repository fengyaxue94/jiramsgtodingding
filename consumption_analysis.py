...
class JiraParse:
    ...
    def new_data_format(self):
        ...
        str1 = '优先级为 {priority} 的issue共有 {number}个。 '
        for each in d:
            str1_2 = str1.format(priority=each, number=d[each]['number'])
            str_0 += str1_2 + '\n'
        final += str_0
        return final
    def new_data_format_solved(self):
        """
        构造发送字符串
        :return:
        """
        ...
        d = {'紧急': {'number': 0, 'keys': []}, '高': {'number': 0, 'keys': []},
             ...
             '最低': {'number': 0, 'keys': []}}
        # 调用search_issues方法，通过设置查询语句bug_jql查询所有的业务需求数量
        all_solved_issues = self.search_issues(self.all_solved_jql.format(project=self.project))
        # 遍历所有已经解决的需求，按优先级分别查询
        for each in all_solved_issues:
            priority = each.fields.priority
            priority_str = str(priority)
            key = each.key
            if d[priority_str]['number']:
                ...
                d[priority_str]['number'] = 1
                d[priority_str]['keys'].append(key)
        str_0 = '{project} 需求项目当前所有关闭&&交付的issue共有{number}个 ：\n'.format(project=self.project, number=str(len(all_solved_issues)))
        str1 = '{priority}优先级关闭&&交付 的issue共有 {number}个。 '
        for each in d:
            str1_2 = str1.format(priority=each, number=d[each]['number'])
            str_0 += str1_2 + '\n'
        final += str_0
    ...
    def new_data_format_unsolved(self):
        """
        构造发送字符串
        :return:
        """
...
if __name__ == '__main__':
    ...
    msgtext = j.new_data_format()+ '\n' + j.new_data_format_solved()+'\n' +j.new_data_format_unsolved()
    dingurls = get_url()
    msgs = {
        "msgtype": "text",
        "text": {
