...
class JiraParse:
    ...
    def __init__(self, name, password, server, project, max_results):
        ...
        self.issues_all_jql = 'project = {project} AND issuetype = 业务需求 AND status in  (草稿,"方案todo","需求todo",待排期,已排期,已交付,关闭 )'
    ...
    def new_data_format(self):
        ...
        d = {'链家': {'number': 0, 'keys': []}, '智联': {'number': 0, 'keys': []},
             'OPPO': {'number': 0, 'keys': []}, '一汽': {'number': 0, 'keys': []},
             '三一': {'number': 0, 'keys': []},'上海电气': {'number': 0, 'keys': []}, '中南': {'number': 0, 'keys': []},
             '中烟': {'number': 0, 'keys': []}, '九阳': {'number': 0, 'keys': []},
             '保时捷': {'number': 0, 'keys': []},'大疆': {'number': 0, 'keys': []}, '德邦': {'number': 0, 'keys': []},
             '歌尔声学': {'number': 0, 'keys': []}, '河北烟草': {'number': 0, 'keys': []},
             '浙交投': {'number': 0, 'keys': []},'猿辅导': {'number': 0, 'keys': []}, '美团': {'number': 0, 'keys': []},
             '花生好车': {'number': 0, 'keys': []}, '长城': {'number': 0, 'keys': []},'迈瑞': {'number': 0, 'keys': []},
             '友邦': {'number': 0, 'keys': []},}
        issues = self.search_issues(self.issues_all_jql.format(project=self.project))
        for each in issues:
            customer = each.fields.关联客户
            customer_str = str(customer)
            key = each.key
            if d[customer_str]['number']:
                d[customer_str]['number'] += 1
                d[customer_str]['keys'].append(key)
            else:
                d[customer_str]['number'] = 1
                d[customer_str]['keys'].append(key)

        str_0 = '{project} 需求项目当前所有的issue共有{number}个 ：\n'.format(project=self.project, number=str(len(issues)))
        str1 = '关联客户为 {customer} 的issue共有 {number}个。 '
        str2 = 'issue编号为： '

        for each in d:
            str1_2 = str1.format(priority=each, number=d[each]['number'])
            str_0 += str1_2
            if each in ['链家', '智联'] and d.get(each)['number'] != 0:
